...
el_utm =
  ...
  where
    trans = goRight 0 '?' '?' 1 ++
            ...
            -- change remaining bits to '_'
            loopRight 205 "01,." ++
            goRight 205 '#' '#' 220 ++
            -- we shouldn't always skip the first input (@@@ + leftend)
            -- because we'll have to update there as well eventually
            -- so I'm looking for the case when we met a/b (because current state will consist of a/b)
	    ...
	    -- now we are at the direction bit (a/b)
